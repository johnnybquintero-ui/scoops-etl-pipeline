...
def main():
    ...
    flavours_raw_df = pd.read_csv(
        raw_dir / "flavours.csv"
    )
    stores_raw_df = pd.read_csv(
        raw_dir / "stores.csv"
    )
    ...
    flavours_clean_df.to_csv(
        cleaned_dir / "flavours.csv",
        index=False,
    )
    stores_clean_df.to_csv(
        cleaned_dir / "stores.csv",
        index=False,
    )
    sales_clean_df.to_csv(
        cleaned_dir / "sales.csv",
        index=False,
    )
    ...
    logging.info("Starting model stage")
    ...
    logging.info(
        "Extract, clean and model stages completed successfully"
    )
